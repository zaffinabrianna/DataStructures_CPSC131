...
namespace CPSC131::BookStore
{
	...
	{
		account_balance_ += adjustment; //Assigns the adjustment to the account balance
	}
	...
	{
		return account_balance_; //returns the account balance
	}
	...
	void BookStore::printInventory() const
	{
		...
		for (auto it = bookList.begin(); it != bookList.end(); ++it)
		{
		cout << "\"" << (*it).getTitle() << "\", by " << (*it).getAuthor() << " [" << (*it).getIsbn() << "] (" << (*it).getStockAvailable() << " in stock)" << endl;
		}
	}
	...
	void BookStore::sellToCustomer(Book& book, size_t price_cents, size_t quantity)
	{
		...
		{
		throw std::range_error("Quantity exceeds stock amount.");
		}
...
/*ookStore.cpp: In member function ‘size_t CPSC131::BookStore::BookStore::getBookStockAvailable(std::string) const’:
BookStore.cpp:91:30: error: ‘class CPSC131::DoublyLinkedList::DoublyLinkedList<CPSC131::BookStore::Book>::Iterator’ has no member named ‘getStockAvailable’
   91 |                 return *(it).getStockAvailable();
      |                              ^~~~~~~~~~~~~~~~~
BookStore.cpp: In member function ‘void CPSC131::BookStore::BookStore::purchaseInventory(const CPSC131::BookStore::Book&)’:
BookStore.cpp:127:23: error: ‘class CPSC131::DoublyLinkedList::DoublyLinkedList<CPSC131::BookStore::Book>::Iterator’ has no member named ‘adjustStockAvailable’
  127 |                 *(it).adjustStockAvailable(book.getStockAvailable());
      |                       ^~~~~~~~~~~~~~~~~~~~
BookStore.cpp: In member function ‘void CPSC131::BookStore::BookStore::printInventory() const’:
BookStore.cpp:173:39: error: ‘class CPSC131::DoublyLinkedList::DoublyLinkedList<CPSC131::BookStore::Book>::Iterator’ has no member named ‘getTitle’
  173 |                 cout << "\"" << *(it).getTitle() << "\", by " << *(it).getAuthor() << " [" << *(it).getIsbn() << "] (" << *(it).getStockAvailable() << " in stock)" << endl;
      |                                       ^~~~~~~~
BookStore.cpp:173:72: error: ‘class CPSC131::DoublyLinkedList::DoublyLinkedList<CPSC131::BookStore::Book>::Iterator’ has no member named ‘getAuthor’; did you mean ‘getCursor’?
  173 |      cout << "\"" << *(it).getTitle() << "\", by " << *(it).getAuthor() << " [" << *(it).getIsbn() << "] (" << *(it).getStockAvailable() << " in stock)" << endl;
      |                                                             ^~~~~~~~~
      |                                                             getCursor
BookStore.cpp:173:101: error: ‘class CPSC131::DoublyLinkedList::DoublyLinkedList<CPSC131::BookStore::Book>::Iterator’ has no member named ‘getIsbn’
  173 | tTitle() << "\", by " << *(it).getAuthor() << " [" << *(it).getIsbn() << "] (" << *(it).getStockAvailable() << " in stock)" << endl;
      |                                                             ^~~~~~~

BookStore.cpp:173:129: error: ‘class CPSC131::DoublyLinkedList::DoublyLinkedList<CPSC131::BookStore::Book>::Iterator’ has no member named ‘getStockAvailable’
  173 | t).getAuthor() << " [" << *(it).getIsbn() << "] (" << *(it).getStockAvailable() << " in stock)" << endl;
      |                                                             ^~~~~~~~~~~~~~~~~

make: *** [Makefile:127: BookStore.o] Error 1
bri@bri-VirtualBox:~/Documents/project-2$ 
*/
